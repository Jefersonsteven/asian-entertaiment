...
export async function GET(request: NextRequest) {
    ...
    const params = {
        ...
        recents: url.searchParams.get('recents')
    }
    ...
    try {
        ...
        const dateCurrent = date.toISOString().split('T')[0]
        const dateTwoMonths = new Date(date.setMonth(date.getMonth() - 1)).toISOString().split('T')[0]
        if(params.next) {
            date = new Date()
        ...
        const responsePrincipal = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=${params.country || 'KR'}${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
        const dataPrincipal = await responsePrincipal.json()
        
        if(!params.country) {
            const responseCN = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=CN${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseTH = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=TH${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseJP = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=JP${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseVN = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=VN${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responsePH = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=PH${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseTW = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=TW${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseID = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=ID${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
            const responseHK = await fetch(`${API_TMDB_URL}/discover/${params.type || 'tv'}?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${params.page || 1}&sort_by=${params.sort || 'popularity.desc'}&with_origin_country=HK${params.genres ? `&with_genres=${params.genres}` : ''}${params.status ? `&with_status${params.status}` : ''}${params.next ? `&first_air_date.gte=${date}${date}${params.recents ? `&first_air_date.gte=${dateTwoMonths}&first_air_date.lte:${dateCurrent}&first_air_date_year=${new Date().getFullYear()}` : ""}` : ''}`, options)
